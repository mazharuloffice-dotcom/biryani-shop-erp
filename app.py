...
# --- ১. মেনু ড্রপডাউন সেটআপ (Menu Dropdown Setup) ---
st.sidebar.title("Swapnajatra Biryani Bari")
menu = st.sidebar.selectbox(
    "Select Navigation Menu",
    ["Dashboard", "Daily Expenses", "Fixed Asset Investment", "Partner Equity"]
)

# --- ২. ফিক্সড অ্যাসেট ইনভেস্টমেন্ট সেকশন (Fixed Asset Investment Cost Code) ---
if menu == "Fixed Asset Investment":
    st.header("🏢 Fixed Asset Investment Cost")
    st.write("Record your long-term capital assets and setup investments here.")

    # ইনপুট ফর্ম
    with st.form("asset_form", clear_on_submit=True):
        col1, col2 = st.columns(2)
        with col1:
            asset_name = st.text_input("Asset Name / Description", placeholder="e.g., Refrigerator, Oven, Furniture")
            asset_category = st.selectbox("Category", ["Kitchen Equipment", "Furniture & Fixtures", "Renovation/Interior", "Pos System/Electronics", "Others"])
        with col2:
            asset_cost = st.number_input("Investment Cost (BDT)", min_value=0.0, step=100.0, format="%.2f")
            purchase_date = st.date_input("Purchase Date")
        
        submit_asset = st.form_submit_with_button("Save Asset Investment")

    # ডাটাবেজ বা স্টেট ট্র্যাকিং (সিম্পল ডেমো স্টোরেজ)
    if "fixed_assets" not in st.session_state:
        st.session_state.fixed_assets = []

    if submit_asset:
        if asset_name and asset_cost > 0:
            st.session_state.fixed_assets.append({
                "Asset Name": asset_name,
                "Category": asset_category,
                "Cost (BDT)": asset_cost,
                "Date": purchase_date.strftime("%Y-%m-%d")
            })
            st.success(f"Successfully added: {asset_name} - {asset_cost} BDT")
        else:
            st.error("Please provide a valid asset name and cost amount.")

    # ডাটা টেবিল ও টোটাল সামারি দেখানো
    if st.session_state.fixed_assets:
        df_assets = pd.DataFrame(st.session_state.fixed_assets)
        st.subheader("Asset List Summary")
        st.dataframe(df_assets, use_container_width=True)
        
        # টোটাল ইনভেস্টমেন্ট হিসাব
        total_investment = df_assets["Cost (BDT)"].sum()
        st.metric(label="Total Fixed Asset Investment", value=f"{total_investment:,.2f} BDT")
    else:
        st.info("No fixed assets recorded yet.")

# বাকি মেনুগুলোর স্ট্রাকচার (আপনার এক্সিস্টিং কোড অনুযায়ী সেট করবেন)
elif menu == "Dashboard":
    st.header("📊 Business Dashboard")
    # Your existing dashboard code here...
    
elif menu == "Daily Expenses":
    st.header("💸 Daily Expense Logging")
    # Your existing daily expense code here...

elif menu == "Partner Equity":
    st.header("🤝 Partner Investments")
    # Your existing partner equity code here...